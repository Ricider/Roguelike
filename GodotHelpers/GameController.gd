...
func _refresh_hand():
	...
	for _g in hand_grouped:
		...
		if idx == selected_card_idx:
			btn.modulate = Color(1, 1, 0.92)
			btn.add_theme_font_size_override("font_size", 28)
			btn.add_theme_color_override("font_color", Color(1, 1, 1))
			# Selected card: bright border + pulsing scale/glow animation
			var _sel_sb: StyleBoxFlat = StyleBoxFlat.new()
			_sel_sb.bg_color = Color(0.22, 0.20, 0.32, 1)
			_sel_sb.border_color = Color(0.95, 0.85, 0.4, 1)
			_sel_sb.set_border_width_all(3)
			_sel_sb.set_corner_radius_all(12)
			_sel_sb.shadow_color = Color(0.95, 0.85, 0.4, 0.45)
			_sel_sb.shadow_size = 8
			btn.add_theme_stylebox_override("normal", _sel_sb)
			btn.add_theme_stylebox_override("hover", _sel_sb)
			...
			_glow_tw.tween_property(btn, "modulate", Color(1, 0.96, 0.55, 1), 0.5)
